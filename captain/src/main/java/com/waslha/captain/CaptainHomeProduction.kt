...
fun CaptainHomeProduction(session: CaptainSession, onLogout: () -> Unit) {
    ...
    suspend fun refresh(showLoading: Boolean = false) {
        ...
        runCatching { repository.me() }
            .onSuccess { response -> if (response.success) driver = response.data else response.message?.let { scope.launch { snackbar.showSnackbar(it) } } }
            .onFailure { reportError(it, "تعذر تحميل بيانات الكابتن") }
        runCatching { repository.trips() }
            .onSuccess { response ->
                if (response.success) {
                    trips = response.data.orEmpty().sortedByDescending { it.updatedAt }
                    active = trips.firstOrNull { it.status in ActiveStatuses }
                } else response.message?.let { scope.launch { snackbar.showSnackbar(it) } }
            }
            .onFailure { reportError(it, "تعذر تحميل الرحلات") }
        if (driver?.available == true && active == null) {
            runCatching { repository.availableTrips() }
                .onSuccess { response -> if (response.success) availableTrips = response.data.orEmpty() else response.message?.let { scope.launch { snackbar.showSnackbar(it) } } }
                .onFailure { reportError(it, "تعذر تحديث الطلبات") }
        } else availableTrips = emptyList()
        loading = false
    }
    ...
    val permissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { result ->
        ...
        else scope.launch { snackbar.showSnackbar("السماح بالموقع مطلوب لتشغيل التتبع المباشر") }
    }
    ...
    LaunchedEffect(candidate?.id, active?.id) {
        ...
        runCatching {
            ...
            repeat(3) { tone.startTone(ToneGenerator.TONE_PROP_BEEP, 300); delay(450) }
            tone.release()
        }
        ...
        Box(Modifier.fillMaxSize().padding(padding)) {
            when (tab) {
                0 -> CaptainDashboard(
                    ...
                    location = location,
                    active = active,
                    available = availableTrips.filter { it.id !in rejectedIds },
                    ...
                    onToggle = {
                        ...
                        scope.launch {
                            runCatching { repository.availability(next) }
                                .onSuccess { response -> if (response.success) driver = response.data else response.message?.let { snackbar.showSnackbar(it) } }
                                .onFailure { reportError(it, "تعذر تغيير حالة الاتصال") }
                            busy = false
                        }
                    },
                    onAccept = { id ->
                        ...
                        scope.launch {
                            runCatching { repository.acceptTrip(id) }
                                .onSuccess { response -> if (response.success) { active = response.data; incoming = null; refresh() } else response.message?.let { snackbar.showSnackbar(it) } }
                                .onFailure { reportError(it, "تعذر قبول الطلب") }
                            busy = false
                        }
                    },
                    onStatus = { id, status ->
                            ...
                            runCatching { repository.updateTripStatus(id, status) }
                                .onSuccess { response ->
                                    if (response.success) {
                                        ...
                                        if (status == "completed" && updated != null) { completed = updated; active = null } else active = updated
                                        refresh()
                                    } else response.message?.let { snackbar.showSnackbar(it) }
                                }
                                .onFailure { reportError(it, "تعذر تحديث حالة الرحلة") }
                            busy = false
                        }
                    },
    ...
    incoming?.let { trip ->
        ModalBottomSheet(onDismissRequest = { incoming = null }) {
            IncomingOrder(trip, seconds, busy,
                onReject = { rejectedIds = rejectedIds + trip.id; incoming = null },
                onAccept = {
                    busy = true
                    scope.launch {
    ...
    completed?.let { trip ->
        ReceiptDialog(trip, busy, onLater = { completed = null }) { score ->
            busy = true
            scope.launch {
                runCatching { repository.rateTrip(DriverRatingRequest(trip.id, trip.customerId, driver?.id.orEmpty(), score)) }
                    .onSuccess { response -> if (response.success) completed = null else response.message?.let { snackbar.showSnackbar(it) } }
                    .onFailure { reportError(it, "تعذر حفظ التقييم") }
                busy = false
            }
        }
    }
    ...
    chatTrip?.let { trip ->
        ChatDialog(trip, messages, messageText, busy, { messageText = it }, { chatTrip = null; messageText = "" }) {
            val text = messageText.trim()
            if (text.isNotEmpty()) {
                busy = true
                scope.launch {
                    runCatching { repository.sendMessage(trip.id, text) }
                        .onSuccess { response -> if (response.success && response.data != null) { messages = (messages + response.data).distinctBy { it.id }; lastMessageAt = maxOf(lastMessageAt, response.data.createdAt); messageText = "" } else response.message?.let { snackbar.showSnackbar(it) } }
                        .onFailure { reportError(it, "تعذر إرسال الرسالة") }
                    busy = false
                }
            }
        }
...
private fun CaptainDashboard(driver: Driver?, location: Coordinates?, active: Trip?, available: List<Trip>, earnings: Int, completedCount: Int, busy: Boolean, onToggle: () -> Unit, onAccept: (String) -> Unit, onStatus: (String, String) -> Unit, onCall: (String) -> Unit, onChat: (Trip) -> Unit, onNavigate: (Coordinates) -> Unit, onAccount: () -> Unit) {
    ...
    Box(Modifier.fillMaxSize()) {
            ...
            Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        ...
                        Text(if (online) "متصل الآن • GPS مباشر" else "غير متصل • لن تستقبل طلبات", color = if (online) Green else Muted, fontSize = 9.sp)
                    }
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    StatCard("أرباح اليوم", "$earnings ل.س")
                    StatCard("رحلات مكتملة", completedCount.toString())
                    StatCard("التقييم", String.format(Locale.US, "%.1f", driver?.rating ?: 0.0))
                }
            }
        }
        ...
        when {
            active != null -> ActiveTrip(active, busy, onStatus, onCall, onChat, onNavigate)
            !online -> ActionCard(Modifier.align(Alignment.BottomCenter), "ابدأ استقبال الرحلات", "فعّل ONLINE حتى يبدأ استقبال الطلبات وتتبع الموقع.", "تشغيل ONLINE", busy, onToggle)
            available.isEmpty() -> ActionCard(Modifier.align(Alignment.BottomCenter), "أنت متصل", "لا توجد طلبات حالياً. يتم التحديث تلقائياً.", null, false) {}
        }
    }
}
...
private fun RowScope.StatCard(label: String, value: String) {
    Surface(Modifier.weight(1f), color = Mint, shape = RoundedCornerShape(15.dp)) { Column(Modifier.padding(horizontal = 7.dp, vertical = 8.dp), horizontalAlignment = Alignment.CenterHorizontally) { Text(value, color = Dark, fontSize = 12.sp, fontWeight = FontWeight.Black, textAlign = TextAlign.Center); Text(label, color = Muted, fontSize = 7.sp, textAlign = TextAlign.Center) } }
}

@Composable
private fun ActionCard(modifier: Modifier, title: String, subtitle: String, button: String?, busy: Boolean, action: () -> Unit) {
    Surface(modifier.padding(13.dp).fillMaxWidth(), color = White.copy(alpha = .97f), shape = RoundedCornerShape(25.dp), shadowElevation = 10.dp) {
...
@Composable
private fun ActiveTrip(trip: Trip, busy: Boolean, onStatus: (String, String) -> Unit, onCall: (String) -> Unit, onChat: (Trip) -> Unit, onNavigate: (Coordinates) -> Unit) {
    ...
    Surface(Modifier.fillMaxWidth().padding(13.dp), color = White.copy(alpha = .98f), shape = RoundedCornerShape(25.dp), shadowElevation = 11.dp) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
            Text(title, color = Ink, fontSize = 16.sp, fontWeight = FontWeight.Black)
            Text("${trip.customerName ?: "الزبون"} • ${trip.distanceKm} كم • ${trip.durationMin} دقيقة • ${trip.estimatedFare} ل.س", color = Muted, fontSize = 9.sp)
...
@Composable
private fun IncomingOrder(trip: Trip, seconds: Int, busy: Boolean, onReject: () -> Unit, onAccept: () -> Unit) {
    Column(Modifier.fillMaxWidth().padding(18.dp).navigationBarsPadding(), verticalArrangement = Arrangement.spacedBy(11.dp)) {
        Row(verticalAlignment = Alignment.CenterVertically) {
            Column(Modifier.weight(1f)) { Text("رحلة جديدة", color = Ink, fontSize = 22.sp, fontWeight = FontWeight.Black); Text("طلب مباشر • 15 ثانية للتفاعل", color = Muted, fontSize = 10.sp) }
            Box(Modifier.size(58.dp), contentAlignment = Alignment.Center) { CircularProgressIndicator(progress = { seconds / 15f }, Modifier.fillMaxSize(), color = if (seconds <= 5) Red else Green, strokeWidth = 5.dp); Text(seconds.toString(), color = Ink, fontSize = 17.sp, fontWeight = FontWeight.Black) }
        }
        Card(Modifier.fillMaxWidth().height(155.dp), shape = RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(Color(0xFFE5ECE8))) { CaptainLiveMap(Modifier.fillMaxSize(), null, trip.pickup, trip.destination) }
        Surface(color = Bg, shape = RoundedCornerShape(18.dp), modifier = Modifier.fillMaxWidth()) {
            Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text("الاستلام", color = Muted, fontSize = 8.sp); Text(coordText(trip.pickup), color = Ink, fontSize = 11.sp, fontWeight = FontWeight.Bold)
                Text("الوجهة", color = Muted, fontSize = 8.sp); Text(coordText(trip.destination), color = Ink, fontSize = 11.sp, fontWeight = FontWeight.Bold)
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { Pill("${trip.distanceKm} كم"); Pill("${trip.durationMin} دقيقة"); Pill("${trip.estimatedFare} ل.س") }
            }
        }
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            OutlinedButton(onClick = onReject, enabled = !busy, modifier = Modifier.weight(1f), shape = RoundedCornerShape(15.dp)) { Text("رفض", color = Red, fontWeight = FontWeight.Bold) }
            Button(onClick = onAccept, enabled = !busy, modifier = Modifier.weight(1f), colors = ButtonDefaults.buttonColors(containerColor = Green), shape = RoundedCornerShape(15.dp)) { Text("قبول الطلب", color = White, fontWeight = FontWeight.Black) }
        }
    }
}

@Composable private fun Pill(text: String) { Surface(Modifier.width(92.dp), color = Mint, shape = RoundedCornerShape(999.dp)) { Text(text, Modifier.padding(horizontal = 8.dp, vertical = 7.dp).fillMaxWidth(), color = Dark, fontSize = 8.sp, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center) } }

@Composable
private fun TripHistory(trips: List<Trip>) {
    ...
    Column(Modifier.fillMaxSize().padding(14.dp)) {
        ...
        Text("سجل الرحلات المكتملة والملغاة", color = Muted, fontSize = 9.sp, modifier = Modifier.padding(top = 3.dp, bottom = 12.dp))
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { FilterChip(selected == 0, { selected = 0 }, label = { Text("الرحلات المكتملة") }, modifier = Modifier.weight(1f)); FilterChip(selected == 1, { selected = 1 }, label = { Text("الرحلات الملغاة") }, modifier = Modifier.weight(1f)) }
        Spacer(Modifier.height(12.dp))
        if (filtered.isEmpty()) EmptyHistory() else LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(9.dp)) { items(filtered, key = { it.id }) { HistoryCard(it) } }
    }
}

@Composable private fun EmptyHistory() { Box(Modifier.fillMaxSize().padding(bottom = 45.dp), contentAlignment = Alignment.Center) { Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(9.dp)) { Surface(Modifier.size(100.dp), color = Mint, shape = CircleShape) { Box(contentAlignment = Alignment.Center) { Text("🚕", fontSize = 45.sp) } }; Text("لا توجد رحلات مسجلة بعد", color = Ink, fontSize = 16.sp, fontWeight = FontWeight.Black); Text("ستظهر الرحلات هنا بعد إنهائها", color = Muted, fontSize = 10.sp) } } }

@Composable private fun HistoryCard(trip: Trip) {
    val cancelled = trip.status == "cancelled"
    Surface(color = White, shape = RoundedCornerShape(20.dp), shadowElevation = 2.dp, modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) { Column(Modifier.weight(1f)) { Text("رحلة #${trip.id.takeLast(7)}", color = Ink, fontSize = 13.sp, fontWeight = FontWeight.Black); Text(dateText(trip.updatedAt.takeIf { it > 0 } ?: trip.createdAt), color = Muted, fontSize = 8.sp) }; Surface(color = if (cancelled) SoftRed else Mint, shape = RoundedCornerShape(999.dp)) { Text(if (cancelled) "ملغاة" else "مكتملة", Modifier.padding(horizontal = 9.dp, vertical = 6.dp), color = if (cancelled) Red else Green, fontSize = 8.sp, fontWeight = FontWeight.Bold) } }
...
@Composable
private fun AccountScreen(driver: Driver?, onLogout: () -> Unit, onDocuments: () -> Unit) {
    LazyColumn(Modifier.fillMaxSize(), contentPadding = androidx.compose.foundation.layout.PaddingValues(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        item { Text("حسابي", color = Ink, fontSize = 25.sp, fontWeight = FontWeight.Black); Text("بياناتك ومركبتك ووثائقك", color = Muted, fontSize = 9.sp) }
        item { AccountCard("المعلومات الشخصية", listOf("الاسم" to (driver?.name?.ifBlank { "غير محدد" } ?: "غير محدد"), "الهاتف" to formatPhone(driver?.phone), "التقييم" to String.format(Locale.US, "%.1f / 5", driver?.rating ?: 0.0), "المحفظة" to "${driver?.walletBalance ?: 0} ل.س")) }
        item { AccountCard("المركبة", listOf("النوع" to (driver?.type?.ifBlank { "اقتصادي" } ?: "اقتصادي"), "الموديل" to (driver?.model?.ifBlank { "غير محدد" } ?: "غير محدد"), "اللون" to (driver?.color?.ifBlank { "غير محدد" } ?: "غير محدد"), "اللوحة" to (driver?.plate?.ifBlank { "غير محدد" } ?: "غير محدد"))) }
        item {
            AccountCard("المستندات", emptyList())
            OutlinedButton(onClick = onDocuments, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(14.dp)) { Text("إدارة الوثائق", fontWeight = FontWeight.Bold) }
        }
...
@Composable private fun AccountCard(title: String, rows: List<Pair<String, String>>) { Surface(color = White, shape = RoundedCornerShape(21.dp), shadowElevation = 2.dp, modifier = Modifier.fillMaxWidth()) { Column(Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) { Text(title, color = Ink, fontSize = 13.sp, fontWeight = FontWeight.Black); rows.forEach { row -> Row(Modifier.fillMaxWidth().padding(vertical = 4.dp), verticalAlignment = Alignment.CenterVertically) { Text(row.first, Modifier.weight(1f), color = Muted, fontSize = 9.sp); Text(row.second, color = Ink, fontSize = 10.sp, fontWeight = FontWeight.Bold, textAlign = TextAlign.End) } } } } }

@Composable
private fun ReceiptDialog(trip: Trip, busy: Boolean, onLater: () -> Unit, onRate: (Int) -> Unit) {
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun ChatDialog(trip: Trip, messages: List<TripMessage>, text: String, busy: Boolean, onTextChange: (String) -> Unit, onDismiss: () -> Unit, onSend: () -> Unit) {
    ModalBottomSheet(onDismissRequest = onDismiss) {
        Column(Modifier.fillMaxWidth().height(530.dp).padding(14.dp).navigationBarsPadding()) {
            ...
            LazyColumn(Modifier.weight(1f).fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(7.dp)) { items(messages, key = { it.id }) { message -> Surface(color = if (message.senderRole == "driver") Mint else White, shape = RoundedCornerShape(14.dp), shadowElevation = 1.dp, modifier = Modifier.fillMaxWidth()) { Column(Modifier.padding(10.dp)) { Text(if (message.senderRole == "driver") "أنت" else "الزبون", color = Muted, fontSize = 7.sp); Text(message.text, color = Ink, fontSize = 10.sp) } } } }
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(7.dp), verticalAlignment = Alignment.CenterVertically) { OutlinedTextField(value = text, onValueChange = onTextChange, modifier = Modifier.weight(1f), singleLine = true, placeholder = { Text("اكتب رسالة") }); Button(onClick = onSend, enabled = !busy && text.isNotBlank(), colors = ButtonDefaults.buttonColors(containerColor = Green), shape = RoundedCornerShape(14.dp)) { Text("إرسال", color = White) } }
        }
    }
}

private fun todayTimestamp(updatedAt: Long, createdAt: Long): Boolean {
    val timestamp = updatedAt.takeIf { it > 0 } ?: createdAt
    if (timestamp <= 0) return false
    val now = java.util.Calendar.getInstance()
    val date = java.util.Calendar.getInstance().apply { timeInMillis = timestamp }
    return now.get(java.util.Calendar.YEAR) == date.get(java.util.Calendar.YEAR) && now.get(java.util.Calendar.DAY_OF_YEAR) == date.get(java.util.Calendar.DAY_OF_YEAR)
}

private fun dateText(timestamp: Long): String = if (timestamp <= 0) "التاريخ غير متوفر" else SimpleDateFormat("yyyy/MM/dd • HH:mm", Locale.getDefault()).format(Date(timestamp))
private fun formatPhone(phone: String?): String { val digits = phone.orEmpty().filter(Char::isDigit).removePrefix("963").take(9); if (digits.isEmpty()) return "+963"; return "+963 ${digits.take(2)} ${digits.drop(2).take(3)} ${digits.drop(5).take(4)}".trim() }
