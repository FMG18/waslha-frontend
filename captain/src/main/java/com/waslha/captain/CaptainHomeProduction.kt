...
fun CaptainHomeProduction(session: CaptainSession, onLogout: () -> Unit) {
    ...
    Scaffold(
        ...
        Box(Modifier.fillMaxSize().padding(padding)) {
            when (tab) {
                0 -> CaptainDashboard(driver, location, activeTrip, availableTrips.filter { it.id !in rejectedIds }, todayEarnings, todayCompleted, busy,
                    onToggleOnline = {
                    ...
                    onAccept = { id ->
                        ...
                        scope.launch {
                            runCatching { repository.acceptTrip(id) }
                                .onSuccess { response -> if (response.success) { activeTrip = response.data; incomingTrip = null; refresh() } else response.message?.let { snackbar.showSnackbar(it) } }
                    ...
                    onStatus = { id, status ->
                        ...
                        scope.launch {
                            runCatching { repository.updateTripStatus(id, status) }
                                .onSuccess { response ->
                                    if (response.success) {
                                        if (status == "completed" && response.data != null) { completedTrip = response.data; activeTrip = null }
                                        else activeTrip = response.data
                                        refresh()
                ...
                else -> CaptainAccountScreen(driver, onLogout) { context.startActivity(Intent(context, CaptainDocumentsActivity::class.java)) }
            }
            if (loading) {
                Surface(Modifier.align(Alignment.Center), color = CaptainWhite, shape = RoundedCornerShape(18.dp), shadowElevation = 6.dp) {
                    CircularProgressIndicator(Modifier.padding(24.dp).size(30.dp), color = CaptainGreen, strokeWidth = 3.dp)
                }
            }
        }
    }
    ...
    incomingTrip?.let { trip ->
        ModalBottomSheet(onDismissRequest = { incomingTrip = null }) {
            IncomingTripSheet(trip, secondsLeft, busy,
                onReject = { rejectedIds = rejectedIds + trip.id; incomingTrip = null },
                onAccept = {
                    busy = true
                    scope.launch {
    ...
    completedTrip?.let { trip ->
        RatingDialog(trip, busy, { completedTrip = null }) { score ->
            busy = true
            scope.launch {
                runCatching { repository.rateTrip(DriverRatingRequest(trip.id, trip.customerId, driver?.id.orEmpty(), score)) }
                    .onSuccess { response -> if (response.success) completedTrip = null else response.message?.let { snackbar.showSnackbar(it) } }
    ...
    chatTrip?.let { trip ->
        ChatSheet(trip, messages, messageText, busy,
...
private fun CaptainDashboard(
    driver: Driver?, location: Coordinates?, activeTrip: Trip?, availableTrips: List<Trip>, todayEarnings: Int, todayCompleted: Int, busy: Boolean,
    onToggleOnline: () -> Unit, onAccept: (String) -> Unit, onStatus: (String, String) -> Unit,
    onCall: (String) -> Unit, onChat: (Trip) -> Unit, onNavigate: (Coordinates) -> Unit, onAccount: () -> Unit
) {
    val online = driver?.available == true
    Box(Modifier.fillMaxSize()) {
        CaptainLiveMap(Modifier.fillMaxSize(), location ?: driver?.let { Coordinates(it.lat, it.lng) }, activeTrip?.pickup, activeTrip?.destination)
        Surface(Modifier.padding(12.dp).fillMaxWidth().align(Alignment.TopCenter), color = CaptainWhite.copy(alpha = 0.97f), shape = RoundedCornerShape(22.dp), shadowElevation = 7.dp) {
            Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        Text(driver?.name?.ifBlank { "الكابتن" } ?: "الكابتن", color = CaptainInk, fontSize = 17.sp, fontWeight = FontWeight.Black)
                        Text(if (online) "متصل الآن • GPS مباشر" else "غير متصل • لن تستقبل طلبات", color = if (online) CaptainGreen else CaptainMuted, fontSize = 9.sp)
                    }
                    Button(onClick = onToggleOnline, enabled = !busy, colors = ButtonDefaults.buttonColors(containerColor = if (online) CaptainDark else Color(0xFFE8EFEB), contentColor = if (online) CaptainWhite else CaptainInk), shape = RoundedCornerShape(999.dp)) {
        ...
        Surface(Modifier.padding(top = 152.dp, end = 12.dp).align(Alignment.TopEnd), color = CaptainWhite.copy(alpha = 0.96f), shape = CircleShape, shadowElevation = 5.dp) {
        ...
        when {
            ...
            !online -> BottomNotice("ابدأ استقبال الرحلات", "فعّل ONLINE حتى يبدأ استقبال الطلبات وتتبع موقعك.", "تشغيل ONLINE", !busy, onToggleOnline)
            availableTrips.isEmpty() -> BottomNotice("أنت متصل", "لا توجد طلبات حالياً. التطبيق يفحص الطلبات تلقائياً.", null, false, {})
            availableTrips.isNotEmpty() -> BottomNotice("لديك طلبات متاحة", "افتح إشعار الرحلة القادمة للموافقة أو الرفض.", null, false, {})
        }
    }
}

@Composable
private fun RowScope.CaptainStat(label: String, value: String) {
    Surface(Modifier.weight(1f), color = Color(0xFFE8F5F0), shape = RoundedCornerShape(14.dp)) {
...
@Composable
private fun BoxScope.BottomNotice(title: String, subtitle: String, button: String?, enabled: Boolean, onClick: () -> Unit) {
    Surface(Modifier.align(Alignment.BottomCenter).padding(12.dp).fillMaxWidth(), color = CaptainWhite.copy(alpha = 0.98f), shape = RoundedCornerShape(22.dp), shadowElevation = 9.dp) {
        Column(Modifier.padding(15.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(7.dp)) {
            ...
            if (button != null) Button(onClick = onClick, enabled = enabled, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = CaptainGreen), shape = RoundedCornerShape(14.dp)) { Text(button, color = CaptainWhite, fontWeight = FontWeight.Black) }
        }
    }
}

@Composable
private fun BoxScope.ActiveTripSheet(trip: Trip, busy: Boolean, onStatus: (String, String) -> Unit, onCall: (String) -> Unit, onChat: (Trip) -> Unit, onNavigate: (Coordinates) -> Unit) {
    val nextStatus = when (trip.status) { "driver_assigned" -> "arriving"; "arriving" -> "in_progress"; "in_progress" -> "completed"; else -> "" }
    val title = when (trip.status) { "driver_assigned" -> "في الطريق إلى الزبون"; "arriving" -> "وصلت للزبون"; "in_progress" -> "الرحلة جارية"; else -> "الرحلة" }
    val button = when (trip.status) { "driver_assigned" -> "وصلت للزبون"; "arriving" -> "بدء الرحلة"; "in_progress" -> "إنهاء الرحلة"; else -> "مكتملة" }
    Surface(Modifier.align(Alignment.BottomCenter).padding(12.dp).fillMaxWidth(), color = CaptainWhite.copy(alpha = 0.99f), shape = RoundedCornerShape(23.dp), shadowElevation = 10.dp) {
        Column(Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
            Text(title, color = CaptainInk, fontSize = 16.sp, fontWeight = FontWeight.Black)
            Text("${trip.customerName ?: "الزبون"} • ${trip.distanceKm} كم • ${trip.durationMin} دقيقة • ${trip.estimatedFare} ${trip.currency}", color = CaptainMuted, fontSize = 9.sp)
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
private fun IncomingTripSheet(trip: Trip, seconds: Int, busy: Boolean, onReject: () -> Unit, onAccept: () -> Unit) {
...
private fun CaptainTripsScreen(trips: List<Trip>) {
    ...
    Column(Modifier.fillMaxSize().padding(14.dp)) {
        ...
        if (filtered.isEmpty()) {
            Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) { Text("لا توجد رحلات في هذا القسم", color = CaptainMuted, fontSize = 13.sp) }
        } else {
...
private fun CaptainAccountScreen(driver: Driver?, onLogout: () -> Unit, onDocuments: () -> Unit) {
    LazyColumn(Modifier.fillMaxSize(), contentPadding = androidx.compose.foundation.layout.PaddingValues(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        item { Text("حسابي", color = CaptainInk, fontSize = 25.sp, fontWeight = FontWeight.Black); Text("بياناتك والمركبة والوثائق", color = CaptainMuted, fontSize = 9.sp) }
        item { AccountCard("المعلومات الشخصية") { AccountRow("الاسم", driver?.name?.ifBlank { "غير محدد" } ?: "غير محدد"); AccountRow("الهاتف", formatPhone(driver?.phone)); AccountRow("التقييم", String.format(Locale.US, "%.1f / 5", driver?.rating ?: 0.0)); AccountRow("الرصيد", "${driver?.walletBalance ?: 0} ل.س") } }
        item { AccountCard("المركبة") { AccountRow("النوع", driver?.type?.ifBlank { "اقتصادي" } ?: "اقتصادي"); AccountRow("الموديل", driver?.model?.ifBlank { "غير محدد" } ?: "غير محدد"); AccountRow("اللون", driver?.color?.ifBlank { "غير محدد" } ?: "غير محدد"); AccountRow("رقم اللوحة", driver?.plate?.ifBlank { "غير محدد" } ?: "غير محدد") } }
        item { AccountCard("المستندات") { AccountRow("الهوية", driver?.documents?.identity?.status ?: "pending"); AccountRow("الرخصة", driver?.documents?.license?.status ?: "pending"); AccountRow("أوراق السيارة", driver?.documents?.vehicle?.status ?: "pending"); OutlinedButton(onClick = onDocuments, modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(13.dp)) { Text("إدارة الوثائق", fontWeight = FontWeight.Bold) } } }
        item { Button(onClick = onLogout, modifier = Modifier.fillMaxWidth(), colors = ButtonDefaults.buttonColors(containerColor = CaptainRed), shape = RoundedCornerShape(14.dp)) { Text("تسجيل الخروج", color = CaptainWhite, fontWeight = FontWeight.Black) } }
    }
}
...
private fun AccountCard(title: String, content: @Composable androidx.compose.foundation.layout.ColumnScope.() -> Unit) {
    Surface(color = CaptainWhite, shape = RoundedCornerShape(20.dp), shadowElevation = 2.dp, modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) { Text(title, color = CaptainInk, fontSize = 13.sp, fontWeight = FontWeight.Black); Spacer(Modifier.height(3.dp)); content() }
    }
}
    ...
    AlertDialog(
        ...
        confirmButton = {
            Row(horizontalArrangement = Arrangement.spacedBy(2.dp)) {
                (1..5).forEach { star -> TextButton(onClick = { score = star }, enabled = !busy) { Text(if (star <= score) "★" else "☆", fontSize = 22.sp, color = CaptainGreen) } }
                Button(onClick = { onRate(score) }, enabled = !busy, colors = ButtonDefaults.buttonColors(containerColor = CaptainGreen)) { Text("حفظ", color = CaptainWhite) }
            }
        }
    )
}
...
private fun formatCoordinates(point: Coordinates): String = String.format(Locale.US, "%.5f, %.5f", point.lat, point.lng)
...
private fun navigate(context: Context, point: Coordinates) {
    ...
    runCatching { context.startActivity(Intent(Intent.ACTION_VIEW, google)) }.onFailure {
