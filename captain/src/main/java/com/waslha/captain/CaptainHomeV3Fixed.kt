...
fun CaptainHomeV3Fixed(session: CaptainSession, onLogout: () -> Unit) {
    ...
    val permissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { result ->
        ...
        if (granted) {
            ...
            scope.launch { snackbars.showSnackbar("السماح بالموقع مطلوب لتشغيل التتبع المباشر") }
        }
    }
    ...
    suspend fun refresh(showLoading: Boolean = false) {
        ...
        runCatching { repository.me() }
            .onSuccess { response ->
                ...
                else scope.launch { snackbars.showSnackbar(response.message ?: "تعذر تحميل الحساب") }
            }
        ...
        runCatching { repository.trips() }
            .onSuccess { response ->
                if (response.success) {
                    ...
                    scope.launch { snackbars.showSnackbar(response.message ?: "تعذر تحميل الرحلات") }
                }
            }
    ...
    LaunchedEffect(chatTrip?.id) {
        ...
        while (chatTrip != null) {
            runCatching { repository.messages(chatTrip!!.id, chatLast) }
                .onSuccess { response ->
                    val items = response.data.orEmpty()
                    if (items.isNotEmpty()) {
                        chatMessages = (chatMessages + items).distinctBy { it.id }.sortedBy { it.createdAt }
    ...
    MaterialTheme {
        Scaffold(
            ...
            Box(Modifier.fillMaxSize().padding(padding)) {
                when (tab) {
                    0 -> CaptainDashboardPage(
                        driver = driver,
                        location = location,
                        active = active,
                        available = available.filter { it.id !in rejected },
                        todayEarnings = todayEarnings,
                        todayCompleted = todayCompleted.size,
                        loading = loading,
                        busy = busy,
                        onToggle = {
                            ...
                            scope.launch {
                                runCatching { repository.availability(next) }
                                    .onSuccess { response ->
                                        if (response.success) driver = response.data
                                        else snackbars.showSnackbar(response.message ?: "تعذر تغيير الحالة")
                                    }
                        ...
                        onAccept = { tripId ->
                            busy = true
                            scope.launch {
                                runCatching { repository.acceptTrip(tripId) }
                                    .onSuccess { response ->
                                        if (response.success) {
                        ...
                        onStatus = { tripId, status ->
                            busy = true
                            scope.launch {
                                runCatching { repository.updateTripStatus(tripId, status) }
                                    .onSuccess { response ->
                                        if (response.success) {
                ...
                if (loading) {
                    Surface(Modifier.align(Alignment.Center), color = White.copy(.94f), shape = RoundedCornerShape(18.dp), shadowElevation = 8.dp) {
                        CircularProgressIndicator(Modifier.padding(22.dp).size(28.dp), color = Green, strokeWidth = 3.dp)
                    }
                }
            }
        }
                        ...
                        runCatching { repository.acceptTrip(trip.id) }
                            .onSuccess { response ->
                                if (response.success) {
                                    ...
                                    incoming = null
                                    refresh()
                                } else snackbars.showSnackbar(response.message ?: "تعذر قبول الطلب")
                            }
                    ...
                    runCatching {
                        repository.rateTrip(
                            DriverRatingRequest(
                                ...
                                driverId = driver?.id.orEmpty(),
                                score = score
                            )
                        )
                    }.onSuccess { response ->
                        if (response.success) completedTrip = null
                        else snackbars.showSnackbar(response.message ?: "تعذر حفظ التقييم")
    ...
    chatTrip?.let { trip ->
        ChatSheet(
            ...
            onDismiss = { chatTrip = null; chatText = "" },
            onSend = {
                        ...
                        runCatching { repository.sendMessage(trip.id, text) }
                            .onSuccess { response ->
                                if (response.success && response.data != null) {
                                    chatMessages = (chatMessages + response.data).distinctBy { it.id }
                                    chatLast = maxOf(chatLast, response.data.createdAt)
                                    chatText = ""
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Surface(onClick = onToggle, enabled = !busy, color = if (online) Dark else Color(0xFFEEF2EF), shape = RoundedCornerShape(999.dp)) {
                        Row(Modifier.padding(horizontal = 12.dp, vertical = 9.dp), verticalAlignment = Alignment.CenterVertically) {
                            Box(Modifier.size(8.dp).background(if (online) White else Muted, CircleShape))
                            Spacer(Modifier.width(7.dp))
                            Text(if (online) "ONLINE" else "OFFLINE", color = if (online) White else Ink, fontSize = 9.sp, fontWeight = FontWeight.Black)
                        }
                    }
                }
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    DashboardStat("أرباح اليوم", "$todayEarnings ل.س")
                    DashboardStat("رحلات مكتملة", todayCompleted.toString())
                    DashboardStat("التقييم", String.format(Locale.US, "%.1f", driver?.rating ?: 0.0))
                }
            }
        }
        Surface(Modifier.padding(top = 177.dp, end = 13.dp).align(Alignment.TopEnd), color = White.copy(.96f), shape = CircleShape, shadowElevation = 6.dp) {
...
private fun DashboardStat(label: String, value: String) {
    Surface(Modifier.weight(1f), color = Mint, shape = RoundedCornerShape(15.dp)) {
        Column(Modifier.padding(horizontal = 8.dp, vertical = 8.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            Text(value, color = Dark, fontSize = 13.sp, fontWeight = FontWeight.Black, textAlign = TextAlign.Center)
            Text(label, color = Muted, fontSize = 7.sp, textAlign = TextAlign.Center)
        }
    }
}
...
private fun DashboardActionCard(modifier: Modifier, title: String, subtitle: String, button: String?, busy: Boolean, action: () -> Unit) {
    Surface(modifier.padding(13.dp).fillMaxWidth(), color = White.copy(.97f), shape = RoundedCornerShape(26.dp), shadowElevation = 11.dp) {
        ...
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Column(Modifier.weight(1f)) {
                    ...
                    Text("${trip.customerName ?: "الزبون"} • ${trip.distanceKm} كم • ${trip.durationMin} دقيقة", color = Muted, fontSize = 9.sp)
                }
                Surface(color = Mint, shape = RoundedCornerShape(999.dp)) { Text(statusLabel(trip.status), Modifier.padding(horizontal = 9.dp, vertical = 7.dp), color = Green, fontSize = 8.sp, fontWeight = FontWeight.Bold) }
            }
...
private fun IncomingSheet(trip: Trip, seconds: Int, busy: Boolean, onReject: () -> Unit, onAccept: () -> Unit) {
    Column(Modifier.fillMaxWidth().padding(18.dp).navigationBarsPadding(), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Surface(color = Bg, shape = RoundedCornerShape(20.dp), modifier = Modifier.fillMaxWidth()) {
            Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                MiniRouteMap(trip)
                Text("الاستلام", color = Muted, fontSize = 8.sp)
...
@Composable
private fun MiniRouteMap(trip: Trip) {
    Card(Modifier.fillMaxWidth().height(155.dp), shape = RoundedCornerShape(18.dp), colors = CardDefaults.cardColors(Color(0xFFE5ECE8))) {
        CaptainLiveMap(Modifier.fillMaxSize(), null, trip.pickup, trip.destination)
    }
}

@Composable
private fun InfoPill(text: String) {
    Surface(Modifier.weight(1f), color = Mint, shape = RoundedCornerShape(999.dp)) { Text(text, Modifier.padding(horizontal = 7.dp, vertical = 6.dp), color = Dark, fontSize = 8.sp, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center) }
}
...
private fun EmptyTripsState() {
    Box(Modifier.fillMaxSize().padding(bottom = 50.dp), contentAlignment = Alignment.Center) {
        Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(9.dp)) {
            Surface(Modifier.size(100.dp), color = Mint, shape = CircleShape) { Box(contentAlignment = Alignment.Center) { Text("🚕", fontSize = 45.sp) } }
            Text("لا توجد رحلات مسجلة بعد", color = Ink, fontSize = 16.sp, fontWeight = FontWeight.Black)
            Text("ستظهر الرحلات هنا بعد إنهائها", color = Muted, fontSize = 10.sp)
        }
    }
}
...
private fun TripHistoryCard(trip: Trip) {
    Surface(color = White, shape = RoundedCornerShape(20.dp), shadowElevation = 2.dp, modifier = Modifier.fillMaxWidth()) {
        Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Column(Modifier.weight(1f)) {
                    Text("رحلة #${trip.id.takeLast(7)}", color = Ink, fontWeight = FontWeight.Black, fontSize = 13.sp)
                    Text(formatDate(trip.updatedAt.takeIf { it > 0 } ?: trip.createdAt), color = Muted, fontSize = 8.sp)
                }
                val cancelled = trip.status == "cancelled"
                Surface(color = if (cancelled) SoftRed else Mint, shape = RoundedCornerShape(999.dp)) { Text(statusLabel(trip.status), Modifier.padding(horizontal = 9.dp, vertical = 6.dp), color = if (cancelled) Red else Green, fontSize = 8.sp, fontWeight = FontWeight.Bold) }
            }
...
private fun CaptainAccountPage(driver: Driver?, onLogout: () -> Unit, onDocuments: () -> Unit) {
    val documentItems = listOf(
        "الهوية" to (driver?.documents?.identity?.status ?: "pending"),
        "الرخصة" to (driver?.documents?.license?.status ?: "pending"),
...
private fun ReceiptDialog(trip: Trip, busy: Boolean, onDismiss: () -> Unit, onRate: (Int) -> Unit) {
    ...
    AlertDialog(
        ...
        title = { Text("ملخص الفاتورة", fontWeight = FontWeight.Black) },
        text = {
        ...
        confirmButton = { Button(onClick = { onRate(score) }, enabled = !busy, colors = ButtonDefaults.buttonColors(containerColor = Green)) { Text("حفظ وإنهاء", color = White) } }
    )
}
    ...
    ModalBottomSheet(onDismissRequest = onDismiss) {
        Column(Modifier.fillMaxWidth().height(530.dp).padding(14.dp).navigationBarsPadding()) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(7.dp), verticalAlignment = Alignment.CenterVertically) {
                OutlinedTextField(value = text, onValueChange = onTextChange, Modifier.weight(1f), singleLine = true, placeholder = { Text("اكتب رسالة") })
                Button(onClick = onSend, enabled = !busy && text.isNotBlank(), colors = ButtonDefaults.buttonColors(containerColor = Green), shape = RoundedCornerShape(14.dp)) { Text("إرسال", color = White) }
            }
        }
    }
}
