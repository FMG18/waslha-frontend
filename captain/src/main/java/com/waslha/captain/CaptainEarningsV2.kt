...
private val EarningsBorder = Color(0xFFE0E7E3)
...
fun CaptainEarningsV2(modifier: Modifier, trips: List<Trip>) {
    ...
    val todayStart = remember { Calendar.getInstance().apply {
    ...
    val shown = when (filter) {
        1 -> completed.filter { it.status == "completed" }
    ...
    LazyColumn(
            ...
            Card(Modifier.fillMaxWidth(), shape = RoundedCornerShape(27.dp), colors = CardDefaults.cardColors(EarningsDark)) {
                Column(Modifier.padding(19.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
                    ...
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        EarningsMetric("الكل", formatEarnings(totalIncome))
                        EarningsMetric("المكتملة", completed.size.toString())
                        EarningsMetric("متوسط الرحلة", formatEarnings(average))
                    }
                }
            }
        }
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(7.dp)) {
                FilterChip("الكل", filter == 0) { filter = 0 }
                FilterChip("مكتملة", filter == 1) { filter = 1 }
                FilterChip("جارية", filter == 2) { filter = 2 }
                FilterChip("ملغاة", filter == 3) { filter = 3 }
            }
        }
        item {
        ...
        if (shown.isEmpty()) {
            ...
            items(shown, key = { it.id }) { trip ->
                EarningsTripRow(trip)
            }
        }
    }
}
...
@Composable
private fun EarningsMetric(title: String, value: String) {
    Column(Modifier.weight(1f).background(EarningsWhite.copy(alpha = .08f), RoundedCornerShape(15.dp)).padding(10.dp)) {
