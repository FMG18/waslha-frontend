/* Waslha Maps v4 — Geoapify + MapLibre. Resilient loading, stable point selection, explicit map readiness. */
(function(){
  'use strict';
  let apiKey='';
  ...
  async function loadLib(){
    ...
    const cssUrls=['https://unpkg.com/maplibre-gl@4.7.1/dist/maplibre-gl.css','https://cdn.jsdelivr.net/npm/maplibre-gl@4.7.1/dist/maplibre-gl.css'];
    if(!document.querySelector('link[data-waslha-maplibre-css]')){const c=document.createElement('link');c.rel='stylesheet';c.href=cssUrls[0];c.dataset.waslhaMaplibreCss='1';document.head.appendChild(c)}
    ...
    for(const src of urls){
      if(window.maplibregl)return;
      try{await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.dataset.waslhaMaplibre='1';s.onload=resolve;s.onerror=reject;document.head.appendChild(s)})}catch(e){diag('library','تعذر تحميل MapLibre',src)}
    }
    if(!window.maplibregl)throw Error('تعذر تحميل مكتبة الخريطة. تحقق من اتصال الإنترنت.');
  }
  async function geoReverse(lat,lon){const k=await key();const r=await fetch(`https://api.geoapify.com/v1/geocode/reverse?lat=${lat}&lon=${lon}&lang=ar&limit=1&apiKey=${encodeURIComponent(k)}`);if(!r.ok)throw Error('تعذر قراءة عنوان الموقع');const d=await r.json();return d.features?.[0]?.properties||null}
  async function autocomplete(text,proximity){const k=await key();const q=new URLSearchParams({text,lang:'ar',limit:'6',format:'json',apiKey:k});if(proximity)q.set('bias',`proximity:${proximity[0]},${proximity[1]}`);const r=await fetch(`https://api.geoapify.com/v1/geocode/autocomplete?${q}`);if(!r.ok)throw Error('تعذر البحث عن المكان');const d=await r.json();return d.results||[]}
  async function directions(a,b,mode='drive'){a=norm(a);b=norm(b);if(!a||!b)return null;const k=await key();const q=new URLSearchParams({waypoints:`${a[1]},${a[0]}|${b[1]},${b[0]}`,mode,format:'geojson',apiKey:k});const r=await fetch(`https://api.geoapify.com/v1/routing?${q}`);if(!r.ok)throw Error('تعذر حساب الطريق');const d=await r.json();const f=d.features?.[0],p=f?.properties;return f?{coordinates:f.geometry.coordinates,distanceKm:(p.distance||0)/1000,durationMinutes:Math.max(1,Math.round((p.time||0)/60)),raw:p}:null}
  async function create(el,o={}){
    ...
    try{
      ...
      map.once('load',()=>{document.getElementById('mapCover')?.classList.add('hidden');render();centerPin(mode!=='locked');if(pickup&&dropoff)route();o.onReady?.({pickup,dropoff,mode});setTimeout(()=>map.resize(),80);setTimeout(()=>map.resize(),500)});
      map.on('click',e=>{if(o.interactive!==false)choose([e.lngLat.lng,e.lngLat.lat])});
      map.on('error',e=>{diag('map','Geoapify/MapLibre error',e?.error?.message);if(e?.error?.status&&String(e.error.status).startsWith('4'))document.getElementById('mapCover')?.classList.add('hidden')});
    ...
    }catch(e){diag('fatal','فشل تشغيل الخريطة',e?.message);document.getElementById('mapCover')?.classList.add('hidden');el.innerHTML=`<div class="map-empty"><b>تعذر تشغيل الخريطة</b><span>${String(e.message||e)}</span></div>`;return null}
  }
