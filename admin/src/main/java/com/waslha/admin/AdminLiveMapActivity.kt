...
class AdminLiveMapActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            var snapshot by remember { mutableStateOf<AdminMapSnapshot?>(null) }
            LaunchedEffect(Unit) {
                while (true) {
                    runCatching { AdminApiProvider.api.mapSnapshot() }
                        .onSuccess { if (it.success) snapshot = it.data }
                    delay(8000)
                }
            }
            Scaffold(topBar = { Text("الخريطة الحية — وصلها") }) { padding ->
                AndroidView(
                    ...
                    update = { webView ->
                        ...
                        val payload = JSONObject()
                            ...
                            .put("trips", JSONArray(s.trips.map { tripJson(it) }))
                            .put("drivers", JSONArray(s.drivers.mapNotNull { driverJson(it) }))
                            .toString()
    ...
    private fun tripJson(t: AdminTripDto) = JSONObject().apply {
        if (t.pickup != null && t.destination != null) {
            put("id", t.id)
            put("status", t.status)
    ...
    private fun driverJson(d: AdminDriverDto): JSONObject? {
        val lat = d.lat ?: return null
        val lng = d.lng ?: return null
